...
#table(yt$sted, yt$harvest_year.x)

yt <- c_mess_master |> filter(site_eng == "\"Ytteborg\"" & 
                               !is.na(Id) & 
                                 sted == 2711 &
                                 leach_year==2015)
...
yt |> ggplot(aes(x=afstro_sum, y=meankonc))+
  geom_point()+
  geom_smooth()+
  theme_bw()

yt_ave <- yt |> group_by(date,afstro_sum,afstroemning) |> 
  summarise(mean_conc=mean(meankonc)) |> 
  ungroup() |> mutate(id_val = row_number())

yt_ave |> ggplot(aes(x=afstro_sum, y=mean_conc, col=date))+
  geom_point()+
  geom_smooth()+
  theme_bw()
...
train_gamm <- train|> 
  mutate(drain_sum=afstro_sum,
         ...
         conc=mean_conc)
#Create test set
test  <- anti_join(yt_ave, train, by = 'id_val')|> 
  mutate(drain_sum=afstro_sum,
         conc=mean_conc)
...
train <- train |> 
  ...
  mutate(idx=seq(1,n(),1),
         drain=yt_wea$afstroemning,#if_else(afstroemning<0,0,afstroemning),
         drain_sum=yt_wea$afstro_sum,
